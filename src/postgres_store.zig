...
pub const Store = struct {
    ...
    pub fn migrate(self: *Store) !void {
        ...
        } else if (version != 13) return error.UnsupportedSchemaVersion;
...
test "postgres runtime migrates chat schema from twelve to thirteen" {
    ...
    defer lease.release();
    var result = try postgres.query(lease.conn, "SELECT max(version),(to_regclass('zigcho.chat_messages') IS NOT NULL)::int,(to_regclass('zigcho.chat_channels') IS NOT NULL)::int FROM zigcho.schema_migrations");
    defer result.deinit();
    try std.testing.expectEqual(@as(i32, 13), try result.int(i32, 0, 0));
