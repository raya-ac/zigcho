...
one security fix went in during the live check too. a leftover `lastfm raw` debug line was dumping stable's password credential into journald with the rest of the query string. it's gone. the normal log after authentication keeps the useful user/action/flag details without keeping the secret.
