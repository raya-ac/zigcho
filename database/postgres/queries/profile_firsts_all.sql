...
WITH own_maps AS MATERIALIZED (
    SELECT b.id AS beatmap_key
    FROM zigcho.scores s JOIN zigcho.beatmaps b ON b.md5=s.map_md5
    WHERE s.user_id=$1 AND s.mode=$2 AND s.rank_namespace=$3
      AND s.passed AND s.best AND b.status IN(3,4)
    UNION
    SELECT s.beatmap_id FROM zigcho.lazer_scores s JOIN zigcho.beatmaps b ON b.id=s.beatmap_id
    WHERE s.user_id=$1 AND s.ruleset_id=$2 AND s.rank_namespace=$3
      AND s.passed AND s.best AND b.status IN(3,4)
), candidates AS (
    SELECT s.id, s.user_id, b.id AS beatmap_key, s.score AS score,
           s.pp, 'stable'::text AS client, s.submitted_at
    ...
    SELECT s.id, s.score AS score, s.pp, s.accuracy, s.max_combo,
           ...
           p.client, NULL::text AS mods_json,
           s.passed AND (coalesce(octet_length(s.replay),0)>0 OR EXISTS(
               SELECT 1 FROM zigcho.replay_objects ro WHERE ro.source='stable' AND ro.score_id=s.id
           )) AS has_replay,
           coalesce(nullif(s.star_rating,0),b.star_rating) AS star_rating,
           s.score AS score_without_mods,
           s.score AS legacy_score, p.first_count
    FROM picked p JOIN zigcho.scores s ON p.client='stable' AND s.id=p.id
    JOIN zigcho.beatmaps b ON b.md5=s.map_md5
    UNION ALL
