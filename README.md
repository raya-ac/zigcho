...
PostgreSQL is the final runtime database. I am not keeping SQLite and PostgreSQL alive as two competing sources of truth. The live Debug build still runs on SQLite while the storage methods are ported, but the one-time migration path is in the repository now.

The importer only accepts the current SQLite schema (version 12) and a PostgreSQL database that does not already contain the `zigcho` schema. It checks SQLite integrity and foreign keys first, creates the typed PostgreSQL schema inside a serializable transaction, copies all 13 tables, resets every identity, and compares every table count and the total stored blob bytes before committing. A failed or repeated import cannot leave a half-copied database behind.
...
The runtime pool is bounded at eight libpq connections. The cutover is the next database phase: port every storage operation, import a stopped SQLite snapshot, compare both sides, then start the Debug service on PostgreSQL with the old database retained as the rollback source.
...
- the PostgreSQL schema, bounded pool, and verified one-time importer are done. the live runtime cutover is next; SQLite remains live until every storage operation has PostgreSQL parity and the stopped database copy passes its checks
- chat still needs the complete player and staff command set, moderation actions, channel controls, durable history, and audit-backed admin tools
...
- lazer stays frozen until the remaining Stable database, chat, ranking, website, and operations work is complete; after that it still needs rooms, event streams, multiplayer spectating, and a properly signed public client release
