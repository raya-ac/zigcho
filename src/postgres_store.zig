...
pub const Store = struct {
    ...
    pub fn migrate(self: *Store) !void {
        ...
        } else if (version != 13 and version != 14) return error.UnsupportedSchemaVersion;
        if (version <= 13) {
            try postgres.exec(
                lease.conn,
                "BEGIN;" ++
...
test "postgres runtime migrates chat and ranking schema through fourteen" {
    ...
    defer lease.release();
    var result = try postgres.query(lease.conn, "SELECT max(version),(to_regclass('zigcho.chat_messages') IS NOT NULL)::int,(to_regclass('zigcho.chat_channels') IS NOT NULL)::int,(to_regclass('zigcho.beatmap_rank_requests') IS NOT NULL)::int,(to_regclass('zigcho.beatmap_rank_events') IS NOT NULL)::int FROM zigcho.schema_migrations");
    defer result.deinit();
    try std.testing.expectEqual(@as(i32, 14), try result.int(i32, 0, 0));
