...
PP is calculated from the exact `.osu` file stored with the beatmap by zigcho's own bounded Zig engine. It parses timing points and hit objects directly, calculates versioned strain and performance for all four Stable rulesets, and has separate Relax and Autopilot paths. Its values are zigcho's model, not a bit-for-bit copy of Akatsuki's old calculator. There is no Rust build, FFI boundary, or external calculator in the runtime. A calculation error rejects the score instead of writing a believable-looking zero. Normal, Relax, and Autopilot scores use separate stat rows; changing mods makes stable display the matching plays, total score, ranked score, accuracy, combo, and weighted PP without leaking them into another mode. `zigcho recalc` does the same job against PostgreSQL in one transaction, rebuilds best scores and ranked stats, and leaves the engine version in an operations audit row.
...
You need Zig 0.16.0, SQLite 3 for the rollback/import tools, and PostgreSQL with libpq for the server build. The server, storage, importers, and PP engine are all Zig.
...
- PP is native Zig with pinned Stable fixtures for osu!, taiko, catch, and mania. vanilla is locked for all four rulesets, relax is locked for osu!/taiko/catch, and autopilot is locked to osu!; max combo is checked against real slider, drum-roll, fruit, and hold-note maps; lazer scoring stays frozen with the rest of lazer
