...
pub const latest_version = "2026.823.0";
...
pub fn historyEntryCount() usize {
    var count: usize = 0;
    for (builds) |build| count += build.updates.len;
    return count;
}

pub fn historyManifest() u64 {
    var manifest: u64 = 0;
    for (builds) |build| for (build.updates) |update| {
...
fn writeStream(writer: *std.Io.Writer, include_latest: bool) anyerror!void {
    try writer.writeAll("{\"id\":5,\"name\":\"lazer\",\"is_featured\":true,\"display_name\":\"zigcho!lazer\",\"user_count\":0,\"latest_build\":");
    if (include_latest) try writeBuild(writer, 0, false, false) else try writer.writeAll("null");
...
fn writeEntry(writer: *std.Io.Writer, build: Build, update: Update, index: usize) anyerror!void {
    ...
    try writer.print("{{\"id\":{d},\"repository\":\"raya-ac/zigcho\",\"github_pull_request_id\":null,\"github_url\":null,\"url\":", .{build.id * 100 + @as(i64, @intCast(index))});
...
fn writeBuild(writer: *std.Io.Writer, index: usize, detailed: bool, navigation: bool) anyerror!void {
    ...
    if (index > 0) try writeBuild(writer, index - 1, false, false) else try writer.writeAll("null");
    try writer.writeAll(",\"previous\":");
    if (index + 1 < builds.len) try writeBuild(writer, index + 1, false, false) else try writer.writeAll("null");
    try writer.writeAll("}}");
}
...
pub fn indexJson(allocator: std.mem.Allocator) ![]u8 {
    ...
    for (builds, 0..) |_, index| {
        if (index != 0) try output.writer.writeByte(',');
        try writeBuild(&output.writer, index, true, false);
    }
...
pub fn buildJson(allocator: std.mem.Allocator, stream: []const u8, version: []const u8) !?[]u8 {
    ...
    for (builds, 0..) |build, index| if (std.mem.eql(u8, build.version, version) or (index == 0 and std.mem.eql(u8, version, "latest"))) {
        var output: std.Io.Writer.Allocating = .init(allocator);
        errdefer output.deinit();
        try writeBuild(&output.writer, index, true, true);
...
test "changelog exposes the complete checked in release history" {
    ...
    try std.testing.expectEqual(@as(usize, 10), object.get("builds").?.array.items.len);
    var entries: usize = 0;
    ...
    try std.testing.expectEqualStrings("lazer profiles stop disappearing", object.get("builds").?.array.items[0].object.get("changelog_entries").?.array.items[0].object.get("title").?.string);
    ...
    try std.testing.expectEqualStrings("2026.822.0", parsed_latest.value.object.get("versions").?.object.get("previous").?.object.get("version").?.string);
