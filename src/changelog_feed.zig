...
pub const manifest_url = "https://raw.githubusercontent.com/zigcho/zigcho/main/updates/changelog.json";
pub const update_url_prefix = "https://raw.githubusercontent.com/zigcho/zigcho/main/updates/";
...
fn manifestWithPrependedBuildAt(allocator: std.mem.Allocator, name: []const u8, digest_markdown: []const u8, build_id: i64, timestamp: []const u8) ![]u8 {
    return manifestWithPrependedBuildVersionAt(allocator, name, digest_markdown, build_id, "2026.902.3", timestamp);
}

fn manifestWithPrependedBuild(allocator: std.mem.Allocator, name: []const u8, digest_markdown: []const u8) ![]u8 {
    return manifestWithPrependedBuildAt(allocator, name, digest_markdown, history.fallback_builds[0].id + 1, "2026-09-02T08:00:00+09:30");
}
...
fn manifestWithPrependedUpdates(allocator: std.mem.Allocator, count: usize, digest_markdown: []const u8) ![]u8 {
    ...
    try output.writer.print("{s}\n    {{\"id\":{d},\"version\":\"2026.902.3\",\"display_version\":\"raw only release\",\"created_at\":\"2026-09-02T08:00:00+09:30\",\"updates\":[", .{ base[0..split], history.fallback_builds[0].id + 1 });
    for (0..count) |index| {
...
test "checked in manifest reuses every embedded update without network fanout" {
    ...
    defer parsed.deinit();
    try std.testing.expectEqualStrings(history.fallback_builds[0].display_version.?, parsed.value.object.get("builds").?.array.items[0].object.get("display_version").?.string);
...
test "feed timestamps are canonical and DateTimeOffset compatible" {
    ...
    const manifest = try manifestWithPrependedBuildAt(std.testing.allocator, "2026-08-25-invalid-time.md", markdown, history.fallback_builds[0].id + 1, "2026-99-99Txxxxxxxxx");
    defer std.testing.allocator.free(manifest);
...
test "dynamic changelog swaps atomically and fetch failure keeps the last good feed" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, latest, "\"version\":\"2026.902.3\"") != null);
...
test "dynamic next year build is visible in changelog and news contracts" {
    ...
    const manifest = try manifestWithPrependedBuildVersionAt(std.testing.allocator, name, markdown, history.fallback_builds[0].id + 1, "2027.102.0", "2027-01-02T00:00:00+09:30");
    defer std.testing.allocator.free(manifest);
    var fixture: TestFetcher = .{ .manifest = manifest, .update_name = name, .update_markdown = markdown };
    var feed = Feed.initWithFetcher(std.testing.allocator, std.testing.io, fixture.interface());
    defer feed.deinit();
...
test "remote changelog rejects traversal and digest mismatches without replacing fallback" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, fallback, "\"version\":\"" ++ history.latest_version ++ "\"") != null);
}
