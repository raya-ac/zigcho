...
- lazer development is open again, but the production claim stays honest. login and trusted kai resources now work in the rebuilt QA client, official solo submission is implemented locally, and rooms, durable event streams, multiplayer spectating, ranked lazer statistics, and a properly signed public client release still remain
- Stable PP is pinned and differential-tested for osu!, taiko, catch, and mania. vanilla is locked for all four rulesets, relax is locked for osu!/taiko/catch, and autopilot is locked to osu!; ScoreV2 stays on its score board without touching normal stats; partial fails use only reached objects; lazer submissions stay in their own unranked input and do not alter the closed Stable PP model
