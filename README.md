...
- stable multiplayer rooms now cover the server-side room, play, visible `#lobby` and room-bound `#multiplayer` chat, invite, tournament-viewer, referee, and abort contract; the real two-client installed stable run is still the completion gate
- stable spectating now covers the server-side host, fellow spectator, private channel, frame, failure, host-switch, and disconnect contract; the installed host/viewer run is still the completion gate
