...
The importer only accepts the current SQLite schema (version 19) and a PostgreSQL database that does not already contain the `zigcho` schema. It checks SQLite integrity and foreign keys first, creates the typed PostgreSQL schema inside a serializable transaction, copies all 23 tables, resets every identity, and compares every table count and the total stored blob bytes before committing. A failed or repeated import cannot leave a half-copied database behind.
