...
fn manifestWithPrependedBuildAt(allocator: std.mem.Allocator, name: []const u8, digest_markdown: []const u8, build_id: i64, timestamp: []const u8) ![]u8 {
    return manifestWithPrependedBuildVersionAt(allocator, name, digest_markdown, build_id, "2026.825.0", timestamp);
}

fn manifestWithPrependedBuild(allocator: std.mem.Allocator, name: []const u8, digest_markdown: []const u8) ![]u8 {
    return manifestWithPrependedBuildAt(allocator, name, digest_markdown, 36, "2026-08-25T00:00:00+09:30");
}
...
fn manifestWithPrependedUpdates(allocator: std.mem.Allocator, count: usize, digest_markdown: []const u8) ![]u8 {
    ...
    try output.writer.print("{s}\n    {{\"id\":36,\"version\":\"2026.825.0\",\"display_version\":\"zigcho release 1.1\",\"created_at\":\"2026-08-25T00:00:00+09:30\",\"updates\":[", .{base[0..split]});
    for (0..count) |index| {
...
test "checked in manifest reuses every embedded update without network fanout" {
    ...
    defer parsed.deinit();
    try std.testing.expectEqualStrings("zigcho release 1", parsed.value.object.get("builds").?.array.items[0].object.get("display_version").?.string);
...
test "feed timestamps are canonical and DateTimeOffset compatible" {
    ...
    const manifest = try manifestWithPrependedBuildAt(std.testing.allocator, "2026-08-25-invalid-time.md", markdown, 36, "2026-99-99Txxxxxxxxx");
    defer std.testing.allocator.free(manifest);
...
test "dynamic changelog swaps atomically and fetch failure keeps the last good feed" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, latest, "\"version\":\"2026.825.0\"") != null);
...
test "dynamic next year build is visible in changelog and news contracts" {
    ...
    const manifest = try manifestWithPrependedBuildVersionAt(std.testing.allocator, name, markdown, 36, "2027.102.0", "2027-01-02T00:00:00+09:30");
    defer std.testing.allocator.free(manifest);
    var fixture: TestFetcher = .{ .manifest = manifest, .update_name = name, .update_markdown = markdown };
    var feed = Feed.initWithFetcher(std.testing.allocator, std.testing.io, fixture.interface());
    defer feed.deinit();
