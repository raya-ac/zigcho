...
## previous-best correction and completed release

the initial score fixtures did not cover an existing personal best. `dae9643` now snapshots its rank, score, combo, accuracy and pp inside the score transaction before replacement, and supplies those values to the beatmap chart. first-score fields remain empty. one shared fixture covers first, worse and improved plays across vanilla, Relax, AP and ScoreV2 in SQLite and PostgreSQL. it passed in Debug and ReleaseSafe. this is targeted storage/serializer evidence, not another complete two-server comparison or an installed-client claim.

[gate 33966789517](https://github.com/zigcho/zigcho/actions/runs/33966789517) passed its production build, focused score-chart checks, PostgreSQL query parity, multipart backup round trip and exact-binary boot. the unchanged parent had already passed the full PostgreSQL suite in [gate 33966075941](https://github.com/zigcho/zigcho/actions/runs/33966075941); the final correction did not repeat that entire suite.

`dae96430c344325d0e8efd4c9857d2336bd4ac2f` was activated on 2026-09-05 at 13:00 UTC. artifact and live executable SHA-256 both equal `a334a338176a019f34916b04a4b800f85c6c23918ebba1349ee5d48d7742ffe1`. the fresh 119,808,421-byte backup restored successfully and its full readback matched before switching. an earlier range timeout left the old service untouched; the retry succeeded. schema remains 48 with no invalid indexes or unvalidated constraints, and no pp recalculation ran. all 22 public hosts, status/assets/avatar routes and unauthenticated API rejection were checked after activation. the release summary is posted to Discord; the previous `6581496` release remains intact for rollback.
