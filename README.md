...
Stable screenshots use the real authenticated multipart route now. an online player can upload a PNG or JFIF image up to 4 MiB, get the short filename the client expects, and open the same bytes again under `/ss/{filename}`. each account has a hard 1,000-file and 256 MiB ceiling. the image is stored in PostgreSQL, not inside a release directory, so a deploy does not erase old screenshot links.
...
PP is calculated from the exact `.osu` file stored with the beatmap. vanilla and ScoreV2 use pinned rosu-pp 4.0.1 with Stable scoring enabled, while Relax and Autopilot use Akatsuki's pinned formulas. Failed plays are calculated only over the objects the player reached instead of pretending they finished the map. The old native Zig model stays in the tests as a differential engine, but it cannot write live scores while its estimates are still off. A calculation error rejects the score instead of writing a believable-looking zero. Normal, Relax, and Autopilot scores use separate stat rows; ScoreV2 keeps its own score board and never rewrites any of them. `zigcho recalc` repairs historical PP and rebuilds player stats in one PostgreSQL transaction, then records the exact engine version in the operations audit log.

Beatmaps can still be imported with the local operator command, but stable no longer needs somebody to seed every map by hand. The first leaderboard request fetches the set metadata from Nerinyan and stores it immediately so the leaderboard renders right away. The background thread tries the Akatsuki beatmap service for the archive, falls back to Nerinyan, extracts the exact `.osu` file, and checks its CRC, MD5, map ID, and set ID before it reaches storage. Some old beatmaps predate embedded map IDs; those use the already-verified metadata IDs when the fields are absent, but a file containing conflicting IDs is still rejected. Ranked and approved maps are allowed into normal scoring; qualified and loved maps get a board without changing ranked score or PP. Failed or mismatched downloads stay unsubmitted.
...
The lazer side has local bearer authentication, `/api/v2/me`, per-ruleset user profiles, the login-time friends, blocks, favourites, notifications and chat-ack contracts, mod discovery, beatmapset search and metadata, archive downloads, raw `.osu` downloads, and score submission. Official solo submission uses the same two requests as the pinned client: a short-lived user/map/hash/ruleset token is created first, then exactly one typed score can consume it. Unknown, expired, foreign, reused, hash-mismatched, or ruleset-mismatched tokens cannot write a score. Mods, hit statistics, maximum statistics, pauses, totals, rank, accuracy, combo, and exact modern mod settings are bounded before storage and PP calculation, and the actual nested mod/stat JSON is stored instead of the whole request body. Passed ranked plays now update the shared account's weighted PP, accuracy, score, playcount, hits, and combo. Stable and lazer still count one result per map instead of inflating the same account twice; failed, unranked, and custom-mod plays stay out of ranked PP. The old direct JSON route remains only as a compatibility surface while the real client moves through the official route. It accepts lazer's nested registration fields and raw password login without splitting stable and lazer into separate accounts. Raw lazer passwords are reduced to the same MD5 credential stable sends, then that credential is wrapped with Argon2id in storage. Tokens are random, stored by hash, scoped, expiring, and revocable. Older development databases using the original hash format upgrade themselves after the next successful login.

The custom client changes are checked in under `client/lazer/` against one pinned official osu! commit. Production points API traffic at `api.kai.ovh`. Debug keeps its API on loopback behind the isolated SSH-tunnel QA lane while account, web, and resource links still use the real `kai.ovh` hosts. The resource store keeps its trust boundary instead of accepting the whole internet: HTTPS on `kai.ovh`, `ppy.sh`, and their real subdomains is allowed, while plaintext, lookalike suffixes, data URLs, and relative URLs stay blocked. I rebuilt and opened the arm64 macOS client, signed the QA bundle ad hoc, logged into the isolated schema 22 lane, loaded a ranked map, submitted the official POST then PUT score flow, and watched the stored score come back as the client leaderboard's #1 play. Public avatar resources also load from `https://a.kai.ovh/{id}` without the old blocked-resource error. Windows x64 has a self-contained portable alpha built on an actual Windows runner now. It has its own storage and IPC identity, cannot update over official lazer, carries both source revisions and licences, and ships with complete SHA-256 manifests. It is still unsigned, so Windows may show a SmartScreen warning and I am not pretending it is the final signed installer. The macOS build remains local QA rather than a public release.
...
You need Zig 0.16.0, Rust, SQLite 3 for the rollback/import tools, and PostgreSQL with libpq for the server build. The PP bridge is a pinned static library; it is not a network service and production score submission does not call another server.
...
The importer only accepts the current SQLite schema (version 22) and a PostgreSQL database that does not already contain the `zigcho` schema. It checks SQLite integrity and foreign keys first, creates the typed PostgreSQL schema inside a serializable transaction, copies all 26 tables, resets every identity, and compares every table count and the total stored blob bytes before committing. A failed or repeated import cannot leave a half-copied database behind.
...
Create the official short-lived solo score token:

```sh
curl -X POST http://127.0.0.1:8080/api/v2/beatmaps/75/solo/scores \
  -H 'Authorization: Bearer TOKEN' \
  -d 'version_hash=11111111111111111111111111111111' \
  -d 'beatmap_hash=0123456789abcdef0123456789abcdef' \
  -d 'ruleset_id=0'
```

Then consume the returned ID once with the score:

```sh
curl -X PUT http://127.0.0.1:8080/api/v2/beatmaps/75/solo/scores/SCORE_TOKEN \
  -H 'Authorization: Bearer TOKEN' \
  -H 'Content-Type: application/json' \
  --data '{"rank":"A","total_score":987654,"total_score_without_mods":900000,"accuracy":0.985,"max_combo":321,"ruleset_id":0,"passed":true,"mods":[{"acronym":"RX"}],"statistics":{"great":300,"ok":4,"miss":1},"maximum_statistics":{"great":305},"pauses":[]}'
```
...
- lazer development is open again, but the production claim stays honest. login, trusted kai resources, official solo submission, exact modern-mod PP, and shared ranked stats are deployed now. rooms, durable event streams, multiplayer spectating, and a properly signed public client release still remain
- Stable PP is pinned and differential-tested for osu!, taiko, catch, and mania. vanilla is locked for all four rulesets, relax is locked for osu!/taiko/catch, and autopilot is locked to osu!; ScoreV2 stays on its score board without touching normal stats; partial fails use only reached objects; lazer uses its own exact modern-mod input without changing the closed Stable calculator
...
The repository currently checks packet framing, malformed packets, safe-name handling, stable's two-stage account creation, the complete stable login fingerprint, exact hardware matching, partial-match false-positive guards, restricted presence, hq!osu flags, server-to-client privilege mapping, country numbers and country boards, stored default avatars and their real image signatures, owned config memory, reconnect-safe token polling, owned login snapshots and tokens, real logout, idle expiry, bounded outgoing queues, directional friends, the permanent ID-3 kai friend, login friend lists, friend-only DM blocking, offline mail replay and read state, AFK away replies, presence refreshes, favourite persistence and duplicate responses, beatmap-info grades and API statuses, replay comments and supporter colours, self-directed vanilla/Relax/Autopilot stats updates, stable spectator joins, fellow viewers, private chat and frames, failure relay, host switching and disconnect cleanup, stable multiplayer wire parsing, `#lobby` and `#multiplayer` lifecycle, room-bound chat isolation, match start, load completion, exact score frames, failures, shared skipping, round completion, and recipient scoping, stable score token authorization, committed score ranking charts, bounded score counters, widened accuracy and checksum arithmetic, failed-score stat isolation, one-time joined chat delivery, enforced silence and silenced-target packets, permission-denied commands, moderation audit rows, durable public history, `#announce` write policy, persistent channel locks, the ID-3 `kai` migration and its staff colour bits, player map requests, BN permission checks, distinct nominations, qualification and ranking transitions, veto and admin rollback, frozen staff statuses, Stable map-status output, score-stat rebuilds after map changes, Relax/custom mod isolation and precedence, typed lazer score bodies, official one-use lazer score tokens, exact client score routes, exact modern-mod PP, shared Stable/lazer per-map weighting, ranked lazer stat isolation, login-time social/profile response shapes, lazer form decoding and password compatibility, Argon2id authentication, legacy credential upgrades, scoped token access, revocation, bounded rate-limit windows, exact staff cookie parsing, raw web-password hashing, staff privilege enforcement, same-origin checks, token-bound CSRF, staff appeal and audit persistence, screenshot signatures, names, storage and byte-for-byte recovery, beatmap cover and preview signatures, bounded media caching and persistence, Rijndael block output, CBC padding, multipart duplicate fields and binary false-boundary bytes, stable score decryption, online checksums, beatmap parsing and MD5s, bounded ZIP extraction, archive CRCs, archive storage, stable Direct results, persistent ratings, lazer beatmapset JSON, exact Stable PP fixtures, the native differential model, real slider/hold combo counts, ScoreV2 stat isolation and repair, and allocation failures, best-score/top-50 announcement gates, in-game score announcements, Stable `/np` action parsing and PP replies, pin replacement and per-slice limits, geolocation lookup, PostgreSQL pool bounds, the schema 12 through 22 PostgreSQL upgrade, the PostgreSQL PP rebuild, and the complete 26-table SQLite-to-PostgreSQL inventory. Authentication, database reads, and Bancho polling are exercised together, and every login allocation is failed in turn under the leak checker, including an owned unread message. Multiplayer and spectator packet construction are also walked through allocation failure. Full score/replay and leaderboard runs use fresh migrated databases in `ReleaseSafe`, including repeated mixed-size uploads, personal ranks, displaced best scores, exact-mod boards, friends boards, country boards, and Relax separation. The PostgreSQL importer has also copied a fixture containing every table and each blob type into a fresh database, proved typed `boolean`, `jsonb`, and `bytea` storage, matched counts and bytes, and refused a second import. A separate real-PostgreSQL Store run covers accounts, credentials, tokens, exact HWID restrictions, friends, favourites, map, archive, screenshot, cover, and preview bytes, Stable pass/fail/Relax/ScoreV2 stats, replay recovery, Direct output, global boards, ratings, beatmap info, replay comments, offline mail, public chat history, channel policy, moderation state, ranking workflow, appeals, committed score placement, exact PP recalculation and repair, current shared beatmap JSON, and the one-use lazer score path. the operator backup is also restored into a disposable PostgreSQL database and checked before release.
