...
pub const Store = struct {
    ...
    fn finishPendingRankedStatsRebuild(self: *Store, conn: *postgres.c.PGconn) !void {
        ...
        defer marker.deinit();
        if (marker.rows() != 0) {
            try self.rebuildRankedStats(conn, false);
            try self.recordAllStatsHistoryCurrentWithConnection(conn);
